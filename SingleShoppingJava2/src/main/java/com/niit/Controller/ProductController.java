    ...
    public String addProduct(@ModelAttribute("productModel")ProductModel productModel, HttpServletRequest request,
		...
		if(bindingresult.hasErrors()){
           System.out.println("Binding Result got a error"); }
    	    ...
    	    String path = "H:\\Eclipse Workspace\\SingleShoppingJava2\\src\\main\\webapp\\WEB-INF\\resources\\product\\";
...
//    -------------------------------------------------USER COMMMON----------------------------------------------
	...
	model.addAttribute("cartsize", orderDAO.getOrderListbyname(User).size());
	  ...
	  ArrayList<ProductModel> list=(ArrayList<ProductModel>)productDAO.getProductListbycategory(categoryId);
		Gson gson= new Gson();
